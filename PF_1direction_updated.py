...
def predict(glider_times, initial_positions, heading_angle, currentdata, t, v_kinematic, n_samples, n_g_std, n_c_std, n_t_std, duration):
	...
	#v_c_north *= 0.5
	v_g_east = np.cos(heading_angle) * np.sqrt((v_kinematic ** 2) - ((v_c_east ** 2) + (v_c_north ** 2)))
	v_g_north = np.sin(heading_angle) * np.sqrt((v_kinematic ** 2) - ((v_c_east ** 2) + (v_c_north ** 2)))
...
def run_filter_one(calibration, duration, filterdata, desLong, desLat, glider_times, glider_longs, glider_lats, glider_wpt_longs, glider_wpt_lats, v_kinematic=0.33, n_samples=200, n_g_std=0.01, n_c_std=0.01, n_t_std=0.05, increment = 2):
	...
	for t in range(1, calibration):
		...
		# add in edge case to other functions where it's already at the waypoint (or possibly very close?)
		# maybe switch back to haversine formula?
		if (diff_in_lon == 0):
			if (diff_in_lat == 0):
				heading_angle = 0
			elif (diff_in_lat < 0):
			...
			if (diff_in_lat == 0):
				...
				heading_angle = math.atan(diff_in_lat / diff_in_lon)
		else:
			if (diff_in_lat == 0):
				heading_angle = 0
			else:
		...
		estimated_positions = predict(glider_times, initial_positions, heading_angle, filterdata["currents"], t - 1, v_kinematic, n_samples, n_g_std, n_c_std, n_t_std, duration)
