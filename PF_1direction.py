...
def estimate_positions(glider_times, initial_positions, heading_angle, t=1, v_kinematic=0.29, n_samples=200, n_g_std = 0.0001, n_c_std =0.005):
	'''
	Input: 
...
def run_filter_one(duration, desLong, desLat, glider_times, measured_position, v_kinematic=0.29, n_samples=200, n_g_std=0.005, n_c_std=0.005):
	...
	times = np.arange(0, len(glider_times), 2)
	...
	for t in range(len(glider_times)):
		...
		estimated_positions[0] = estimate_positions(glider_times, initial_positions[0], heading_angle, t=2*t, v_kinematic=0.29)
